...
function initSession() {
    ...
    // Load active APIs (merge static entries with approved user-submitted ones)
    const approvedCustom = JSON.parse(localStorage.getItem('agent_approved_apis') || '[]');
    state.apis = [...APIS_DATA, ...approvedCustom];
    
    // Apply admin price overrides for static seed APIs
    const overrides = JSON.parse(localStorage.getItem('agent_api_overrides') || '{}');
    state.apis.forEach(api => {
        if (overrides[api.name] !== undefined) {
            api.price_per_call = overrides[api.name];
        }
    });
    
    // Refresh stats on page
    updatePlatformStats();

    // Check for saved user session
    const savedUser = localStorage.getItem('agent_user');
    if (savedUser) {
        // Sync local session with the user database
        const usersDb = JSON.parse(localStorage.getItem('agent_users_db'));
        const sessionUser = JSON.parse(savedUser);
        const matched = usersDb.find(u => u.id === sessionUser.id);
        if (matched) {
            state.user = matched;
            localStorage.setItem('agent_user', JSON.stringify(matched));
            updateUserUI();
        } else {
            logout();
        }
...
function executeApiPurchase() {
    if (!state.user || !state.purchaseTargetApi) return;
    
    const api = state.purchaseTargetApi;
    const unlockCost = api.tier === 'Standard' ? 10.00 : 50.00;
    
    if (state.user.balance < unlockCost) {
        showNotification('Purchase Failed', 'Insufficient wallet balance.', 'error');
        return;
    }
    
    // Debit user
    state.user.balance -= unlockCost;
    if (!state.user.purchasedApis) state.user.purchasedApis = [];
    state.user.purchasedApis.push(api.name);
    
    // Save to users database
    const usersDb = JSON.parse(localStorage.getItem('agent_users_db'));
    const idx = usersDb.findIndex(u => u.id === state.user.id);
    if (idx !== -1) {
        usersDb[idx] = state.user;
        localStorage.setItem('agent_users_db', JSON.stringify(usersDb));
        localStorage.setItem('agent_user', JSON.stringify(state.user));
    }
    
    // Log platform revenue
    const rev = parseFloat(localStorage.getItem('agent_revenue') || '0');
    localStorage.setItem('agent_revenue', (rev + unlockCost).toString());
    
    closePurchaseModal();
    updateUserUI();
    showNotification('Purchase Successful', `${api.name} is now unlocked!`, 'success');
    
    // Re-render Explore page to reflect button change
    filterAndRenderApis();
}
...
function handleAuth(type) {
    ...
    if (type === 'register') {
        ...
        if (existing) {
            showNotification('Registration Error', 'An account with this email already exists.', 'error');
            return;
        }
        
        // Generate random API key for LLM agents
        const randomKey = 'agent_key_' + Math.random().toString(36).substring(2, 10) + Math.random().toString(36).substring(2, 10);
        ...
        showNotification('Welcome!', 'Registration successful. 100 PKR added to your wallet.', 'success');
        updateUserUI();
        showView('portal');
    } else {
        // Login
        const matched = usersDb.find(u => u.email.toLowerCase() === email.toLowerCase() && u.password === password);
        if (!matched) {
            showNotification('Login Failure', 'Incorrect email or password.', 'error');
            return;
        }
        ...
        showNotification('Success', `Welcome back, ${matched.email}!`, 'success');
        updateUserUI();
        showView('portal');
    }
}
...
async function runSandboxRequest() {
    ...
    const isUnlocked = isApiSubscribed(api.name);
    
    outputTerminal.textContent = `// Resolving proxy router connection...\n// API Key: ${state.user.apiKey}\n// Wallet Check: Current balance ${state.user.balance.toFixed(2)} PKR\n// Target Cost: ${cost.toFixed(2)} PKR / query...\n`;
    outputTerminal.className = 'code-terminal output';
    
    const latency = Math.floor(Math.random() * 320) + 90;
    
    // Simulate gateway delay
    await new Promise(r => setTimeout(r, 800));
    
    // 1. Check if Premium API is bought/unlocked
    if (api.tier !== 'Free' && !isUnlocked) {
        outputTerminal.innerHTML = `
<span style="color:#ef4444;">&gt; HTTP/1.1 403 Forbidden</span>
<span style="color:#94a3b8;">&gt; Connection: Close</span>
<span style="color:#ef4444;">&gt; Error: API subscription access is locked.</span>

{
  "detail": "Purchase access to standard/pro tier APIs from the marketplace explore catalog before integrating them."
}
        `;
        return;
    }
    ...
    if (state.user.balance < cost) {
        outputTerminal.innerHTML = `
<span style="color:#f59e0b;">&gt; HTTP/1.1 402 Payment Required</span>
<span style="color:#94a3b8;">&gt; Time: ${latency} ms</span>
<span style="color:#f59e0b;">&gt; X-Proxy-Cost: ${cost.toFixed(2)} PKR</span>
<span style="color:#f59e0b;">&gt; X-Agent-Remaining-Balance: ${state.user.balance.toFixed(2)} PKR</span>

{
  "detail": "Wallet balance is below the per-call price of target API. Top up credits immediately using Easypaisa Checkout.",
  "required_balance_pkr": ${cost.toFixed(2)},
  "current_balance_pkr": ${state.user.balance.toFixed(2)}
}
        `;
        
        // Log transaction call as failed
        logProxyCall(api.name, 402, cost);
        return;
    }
    
    // 3. Deduct credit & save database
    ...
    // Log proxy call as success
    logProxyCall(api.name, 200, cost);
    updateBalanceDisplay();
    
    // 4. Generate Mock Data Response or fetch live if CORS enables it
    let body = "";
    let canLiveFetch = (api.tier === 'Free' && api.cors.toLowerCase() === 'yes' && api.auth.toLowerCase() === 'no');
    
    if (canLiveFetch) {
        try {
            // Live fetch sandbox call
            const realRes = await fetch(api.url);
            ...
            body = generateCategoryMock(api.category, api.name);
        }
    } else {
        // Return custom simulation responses based on category
        body = generateCategoryMock(api.category, api.name);
    }
...
<span style="color:#94a3b8;">&gt; Time: ${latency} ms</span>
<span style="color:#10b981;">&gt; X-Proxy-Cost: ${cost.toFixed(2)} PKR</span>
<span style="color:#c084fc;">&gt; X-Agent-Remaining-Balance: ${state.user.balance.toFixed(2)} PKR</span>
<span style="color:#94a3b8;">&gt; Content-Type: application/json</span>

${body}
    `;
    
    showNotification('Credits Deducted', `Charged ${cost.toFixed(2)} PKR for proxy call.`, 'info');
}
...
function handleTopupSubmit() {
    ...
    const epNumber = localStorage.getItem('agent_ep_number');
    const epName = localStorage.getItem('agent_ep_name');
    const orderId = 'EP_TX_' + Date.now().toString().substring(6) + Math.floor(Math.random() * 100);
    
    // Save pending payment record in localStorage
    ...
    // Redirect to easypaisa mock page
    setTimeout(() => {
...
function superAdminLogin() {
    ...
    const db = JSON.parse(localStorage.getItem('agent_users_db'));
    const matched = db.find(u => u.email.toLowerCase() === email.toLowerCase() && u.password === pass && u.isAdmin);
    ...
    if (matched) {
        ...
        showNotification('Security Reject', 'Invalid Super Admin credentials.', 'error');
    }
}
...
function renderSuperAdminOverview() {
    ...
    const rev = parseFloat(localStorage.getItem('agent_revenue') || '0');
    document.getElementById('sa-stat-revenue').textContent = `PKR ${rev.toFixed(2)}`;
    
    // Total users (excluding admin)
    const db = JSON.parse(localStorage.getItem('agent_users_db') || '[]');
    document.getElementById('sa-stat-agents').textContent = db.filter(u => !u.isAdmin).length;
    
    // Pending payments
    const payments = JSON.parse(localStorage.getItem('agent_payments') || '[]');
    const pending = payments.filter(p => p.status === 'pending').length;
    document.getElementById('sa-stat-pending').textContent = pending;
    
    // Total API proxy calls
    const logs = JSON.parse(localStorage.getItem('agent_system_logs') || '[]');
    document.getElementById('sa-stat-calls').textContent = logs.length;
    
    // Active number
    const num = localStorage.getItem('agent_ep_number');
    const name = localStorage.getItem('agent_ep_name');
...
function approvePayment(orderId) {
    ...
    const userIdx = db.findIndex(u => u.email.toLowerCase() === payment.user_email.toLowerCase());
    
    if (userIdx !== -1) {
        db[userIdx].balance += payment.amount;
        payments[payIdx].status = 'approved';
        
        // Log transaction revenue
        const rev = parseFloat(localStorage.getItem('agent_revenue') || '0');
        localStorage.setItem('agent_revenue', (rev + payment.amount).toString());
        
        localStorage.setItem('agent_users_db', JSON.stringify(db));
        localStorage.setItem('agent_payments', JSON.stringify(payments));
        
        showNotification('Payment Approved', `Credited ${payment.amount.toFixed(2)} PKR to ${payment.user_email}`, 'success');
        renderPaymentQueue();
    }
}
...
function rejectPayment(orderId) {
    ...
    payments[payIdx].status = 'rejected';
    localStorage.setItem('agent_payments', JSON.stringify(payments));
    
    showNotification('Payment Rejected', `Order ID ${orderId} has been marked rejected.`, 'info');
    renderPaymentQueue();
}
...
function renderAgentTable() {
    ...
    const filtered = users.filter(u => u.email.toLowerCase().includes(query) && !u.isAdmin);
    ...
    filtered.forEach(usr => {
        const userCalls = logs.filter(l => l.user_email === usr.email);
        const spent = userCalls.reduce((acc, c) => acc + c.cost, 0);
        ...
        tr.innerHTML = `
            ...
            <td>${userCalls.length} calls</td>
            <td>${spent.toFixed(2)} PKR</td>
...
function adjustUserBalancePrompt(userId) {
    ...
    if (idx !== -1) {
        ...
        // Sync log session if admin adjusted current user
...
function saveAdminPriceOverride(apiName) {
    ...
    // Find in active listing
    const idx = state.apis.findIndex(a => a.name === apiName);
    if (idx !== -1) {
        state.apis[idx].price_per_call = newPrice;
        
        // Save price override to localStorage database of custom approvals or standard adjustments
        const approvedList = JSON.parse(localStorage.getItem('agent_approved_apis') || '[]');
        const customIdx = approvedList.findIndex(a => a.name === apiName);
        
        if (customIdx !== -1) {
            approvedList[customIdx].price_per_call = newPrice;
            localStorage.setItem('agent_approved_apis', JSON.stringify(approvedList));
        } else {
            // Overrides dictionary for static seed
            const overrides = JSON.parse(localStorage.getItem('agent_api_overrides') || '{}');
            overrides[apiName] = newPrice;
            localStorage.setItem('agent_api_overrides', JSON.stringify(overrides));
        }
